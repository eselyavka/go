...
func countDays(days int, meetings [][]int) int {
	...
	for i := 1; i < len(meetings); i++ {
		...
		if currStart <= end {
			end = util.MaxInts([]int{end, currEnd})
		} else {
	...
	ans += util.MaxInts([]int{mergedMeetings[0][0] - 1, 0})

	for i := 1; i < len(mergedMeetings); i++ {
		prevEnd := mergedMeetings[i-1][1]
		currStart := mergedMeetings[i][0]
		ans += util.MaxInts([]int{currStart - prevEnd - 1, 0})
	}
	ans += util.MaxInts([]int{days - mergedMeetings[len(mergedMeetings)-1][1], 0})
