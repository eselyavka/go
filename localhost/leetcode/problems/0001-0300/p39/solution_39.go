...
func rec(target, idx, sum int, local_ans, candidates []int, ans *[][]int) {
	...
	rec(target, idx, sum+candidates[idx], local_ans, candidates, ans)

	local_ans = local_ans[:len(local_ans)-1]

	rec(target, idx+1, sum, local_ans, candidates, ans)
}
...
func combinationSum(candidates []int, target int) [][]int {
	...
	rec(target, 0, 0, local_ans, candidates, &ans)
