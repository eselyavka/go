...
func bfs1631(mid int, heights [][]int) bool {
	...
	visited := make(map[util.TupleInt]bool)
	start := util.TupleInt{Row: 0, Col: 0}
	visited[start] = true
	queue := make([]util.TupleInt, 1)
	queue[0] = start
	...
	for len(queue) > 0 {
		...
		for _, item := range adj {
			...
			new_node := util.TupleInt{Row: new_x, Col: new_y}

			if _, ok := visited[new_node]; ok {
				continue
			}

			curr_diff := util.Abs(heights[new_x][new_y] - heights[x][y])
